<div x-data="{ activeTab: @entangle('activeTab') }" class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">Inventory & Stock Tracking</h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-500">Monitor batch levels, expiries, and all stock movements.</p>
        </div>
    </div>
                ...
                <div class="overflow-x-auto -mx-3 sm:-mx-4 md:-mx-6">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Store</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Batch No</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Expiry</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Qty</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $item->store->name }}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $item->batch_no }}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $item->expiry_date->format('Y-m-d') }}</td>
                            <td class="py-3 px-4 text-sm font-bold {{ $item->quantity <= ($item->product->reorder_level ?? 0) ? 'text-red-600' : 'text-gray-900' }}">
                            ...
                            <td class="py-3 px-4 text-sm">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Good</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
                ...
                <div class="overflow-x-auto -mx-3 sm:-mx-4 md:-mx-6">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">In</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Out</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Balance</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="py-3 px-4 text-xs text-gray-500">{{ $move->created_at->format('Y-m-d H:i') }}</td>
                            <td class="py-3 px-4 text-sm text-gray-900 font-medium">{{ $move->product->name }}</td>
                            ...
                            <td class="py-3 px-4 text-sm text-green-600 font-bold">{{ $move->quantity_in > 0 ? '+'.$move->quantity_in : '-' }}</td>
                            <td class="py-3 px-4 text-sm text-red-600 font-bold">{{ $move->quantity_out > 0 ? '-'.$move->quantity_out : '-' }}</td>
                            <td class="py-3 px-4 text-sm text-gray-900 font-semibold">{{ $move->balance }}</td>
                            <td class="py-3 px-4 text-xs text-gray-500">{{ $move->notes }}</td>
                            <td class="py-3 px-4 text-xs text-gray-500">{{ $move->creator->name ?? 'System' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                        </table>
    ...
    <script>
        ...
        // Listen for tab changes from Alpine
        window.addEventListener('resize', () => {
             // Just a trick to re-init if needed during development
        });
    </script>
</div>
