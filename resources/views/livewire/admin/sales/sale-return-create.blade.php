        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-8">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h2 class="text-3xl font-extrabold text-slate-800">{{ __('Sales Return') }}</h2>
                        <p class="text-sm text-slate-500 mt-1">{{ __('Process a product return for an existing sale') }}</p>
                    </div>
                </div>
                ...
                @if(!$sale)
                    <div class="bg-slate-50 p-8 rounded-xl border-2 border-dashed border-slate-200 text-center">
                        <div class="max-w-md mx-auto">
                            ...
                            <h3 class="text-lg font-bold text-slate-700 mb-2">{{ __('Find Sale Invoice') }}</h3>
                            <p class="text-sm text-slate-500 mb-6">{{ __('Enter the invoice number from the receipt to begin the return process.') }}</p>
                            
                            <div class="relative">
                                <input type="text" wire:model.live.debounce.500ms="sale_id" placeholder="{{ __('Invoice # (e.g. SALE-6785...)') }}" 
                        ...
                        <div class="lg:col-span-2 space-y-8">
                            ...
                            <div class="bg-slate-50 p-6 rounded-xl border border-slate-200">
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    ...
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ __('Sale Date') }}</p>
                                        <p class="text-slate-700 font-medium">{{ $sale->sale_date->format('Y-m-d') }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ __('Customer') }}</p>
                                        <p class="text-slate-700 font-medium">{{ $sale->customer->name ?? __('Walk-in') }}</p>
                                    </div>
                                ...
                                <table class="w-full text-left border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ __('Sold / Left') }}</th>
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ __('Returning') }}</th>
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ __('Price') }}</th>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-sm text-slate-600">
                                                {{ $item['sold_quantity'] }} {{ __('Sold') }} / <span class="text-blue-600 font-bold">{{ $item['max_returnable'] }}</span> {{ __('Left') }}
                                            </td>
                                            <td class="px-6 py-4">
                            ...
                            <div class="bg-slate-800 text-white rounded-2xl p-6 shadow-xl shadow-slate-200 relative overflow-hidden">
                                <div class="relative z-10">
                                    <h3 class="text-lg font-bold mb-6 flex justify-between items-center">
                                        {{ __('Return Summary') }}
                                        <span class="text-[10px] font-bold bg-slate-700 px-2 py-1 rounded text-slate-300 uppercase letter tracking-widest">{{ __('Refund') }}</span>
                                    </h3>
