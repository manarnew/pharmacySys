<div x-data="{ showModal: false, isEditing: @entangle('isEditing') }"
    ...
    <div class="mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">{{ __('Expenses') }}</h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-500">{{ __('Track and manage your expenses.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-3 sm:p-4 md:p-6">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <p class="text-sm text-yellow-700 font-medium">
                                {{ __('shifts.no_open_shift_found') }}
                            </p>
                            <p class="mt-1 text-xs text-yellow-600">
                                {{ __('Please open a shift first before adding expenses.') }}
                                <a href="{{ route('admin.shifts.index') }}" wire:navigate class="font-semibold underline hover:text-yellow-800">
                                    {{ __('Go to Shifts') }}
                                </a>
                            </p>
                        </div>
            ...
            <div class="flex justify-end mb-4">
                ...
                @if($currentShift)
                    <button @click="showModal = true; $wire.openModal()" type="button" class="inline-flex items-center rounded-lg bg-blue-600 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200">
                        ...
                        <span class="hidden sm:inline">{{ __('Add Expense') }}</span>
                        <span class="sm:hidden">{{ __('Add') }}</span>
                    </button>
                @else
                    <button type="button" disabled class="inline-flex items-center rounded-lg bg-gray-300 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-gray-500 shadow-sm cursor-not-allowed opacity-60">
                        <svg class="mr-1 sm:mr-2 -ml-1 h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <table id="expensesTable" class="display w-full" style="width:100%">
                <thead>
                    <tr>
                        <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">{{ __('Reason') }}</th>
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                <span x-text="isEditing ? '{{ __('Edit Expense') }}' : '{{ __('Add New Expense') }}'"></span>
                            </h3>
                            <div class="mt-2 text-sm text-gray-500">
                                <div class="grid grid-cols-1 gap-y-4">
                                    <div>
                                        <label for="reason" class="block text-sm font-medium text-gray-700 text-left">{{ __('Reason') }}</label>
                                    ...
                                    <div>
                                        <label for="amount" class="block text-sm font-medium text-gray-700 text-left">{{ __('Amount') }}</label>
                                    ...
                                    <div>
                                        <label for="date" class="block text-sm font-medium text-gray-700 text-left">{{ __('Date') }}</label>
    ...
    <script>
        function initDataTable() {
            ...
            $('#expensesTable').DataTable({
                ...
                language: {
                    search: "",
                    searchPlaceholder: "{{ __('Search expenses...') }}",
                    emptyTable: "{{ __('No expenses found') }}",
                    paginate: {
                        first: "{{ __('First') }}",
                        last: "{{ __('Last') }}",
                        next: "{{ __('Next') }}",
                ...
                initComplete: function() {
                    $('.dataTables_filter input').attr('placeholder', '{{ __('Search expenses...') }}');
                }
            });
        }
        ...
        document.addEventListener('livewire:initialized', () => {
            @foreach(['expense-saved', 'expense-deleted', 'open-edit-modal'] as $event)
                Livewire.on('{{ $event }}', () => {
