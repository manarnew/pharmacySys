        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-8">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h2 class="text-3xl font-extrabold text-slate-800">Purchase Return</h2>
                        <p class="text-sm text-slate-500 mt-1">Process a product return to a supplier</p>
                    </div>
                </div>
                ...
                @if(!$purchase)
                    <div class="bg-slate-50 p-8 rounded-xl border-2 border-dashed border-slate-200 text-center">
                        <div class="max-w-md mx-auto">
                            ...
                            <h3 class="text-lg font-bold text-slate-700 mb-2">Find Purchase Invoice</h3>
                            <p class="text-sm text-slate-500 mb-6">Enter the purchase invoice ID from the supplier to begin the return process.</p>
                            
                            <div class="relative">
                                <input type="text" wire:model.live.debounce.500ms="purchase_id" placeholder="Invoice # (e.g. PUR-6785...)" 
                        ...
                        <div class="lg:col-span-2 space-y-8">
                            ...
                            <div class="bg-slate-50 p-6 rounded-xl border border-slate-200">
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    ...
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Purchase Date</p>
                                        <p class="text-slate-700 font-medium">{{ $purchase->purchase_date->format('Y-m-d') }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Supplier</p>
                                        <p class="text-slate-700 font-medium">{{ $purchase->supplier->name ?? 'Unknown' }}</p>
                                    </div>
                                ...
                                <table class="w-full text-left border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Inventory Status</th>
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Returning</th>
                                            <th class="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Cost</th>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-xs text-slate-600">Purchased: {{ $item['purchased_quantity'] }}</div>
                                                <div class="text-xs font-bold {{ $item['available_in_stock'] > 0 ? 'text-emerald-600' : 'text-red-600' }}">
                                                    In Stock: {{ $item['available_in_stock'] }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                            ...
                            <div class="bg-slate-800 text-white rounded-2xl p-6 shadow-xl shadow-slate-200 relative overflow-hidden">
                                <div class="relative z-10">
                                    <h3 class="text-lg font-bold mb-6 flex justify-between items-center">
                                        Return Summary
                                        <span class="text-[10px] font-bold bg-slate-700 px-2 py-1 rounded text-slate-300 uppercase letter tracking-widest">Supplier</span>
                                    </h3>
